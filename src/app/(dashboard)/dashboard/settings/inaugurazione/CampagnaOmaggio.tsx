...
import { Megaphone, Loader2, Send, X, AlertTriangle, CheckCircle } from 'lucide-react';
...
export default function CampagnaOmaggio() {
  ...
  const apri = async () => {
    ...
    setEsito(null);
    setBusy(true);
    try { setAnteprima(await previewAutomation('omaggio')); }
  ...
  const invia = async () => {
    ...
    if (!window.confirm(
      `Mandare il messaggio a ${quanti} contatt${quanti === 1 ? 'o' : 'i'}?\n\n` +
      'I messaggi partono davvero e non si possono richiamare. Ogni contatto lo riceve una volta sola.'
    )) return;
    setBusy(true);
    try { setEsito(await runAutomationNow('omaggio')); }
    finally { setBusy(false); }
  };
          ...
          <div className="relative z-10 w-full max-w-2xl max-h-[85vh] flex flex-col bg-bg-secondary border border-border rounded-2xl shadow-2xl overflow-hidden">
            <div className="flex items-start justify-between gap-3 px-5 py-4 border-b border-border">
              <div>
                ...
                <p className="text-xs text-text-muted">Va a chi ha scaricato il coupon e non ha ancora prenotato</p>
              </div>
              ...
              {anteprima && !esito && (
                <>
                  ...
                  <p className="text-[11px] text-text-muted leading-relaxed">
                    Chi ha già prenotato, chi ha revocato il consenso marketing e chi ha già ricevuto
                    questa campagna viene saltato in automatico. Serve il template
            ...
            <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-border bg-bg-tertiary/30">
              ...
              {!esito && (
                <button onClick={invia} disabled={busy || !anteprima || anteprima.candidates === 0}
                  ...
                  Invia a {anteprima?.candidates ?? 0}
                </button>
              )}
            </div>
          </div>
