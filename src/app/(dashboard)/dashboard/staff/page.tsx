        ...
        <div className="w-full max-w-xl bg-bg-secondary border border-border rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => { if (window.confirm(`Eliminare ${operator.firstName} ${operator.lastName}?`)) { onDelete(); onClose(); } }}
                className="p-2 rounded-xl hover:bg-error/10 text-text-muted hover:text-error transition-colors" title="Elimina">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <AnimatePresence>
        ...
        {detailOpId && (() => {
          const op = staffList.find(o => o.id === detailOpId);
          if (!op) return null;
          return (
            <StaffDetailModal
              key={op.id}
        ...
        {uscita && (
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
              className="fixed inset-0 z-[71] flex items-center justify-center p-4" onClick={e => daSfondo(e) && setUscita(null)}>
